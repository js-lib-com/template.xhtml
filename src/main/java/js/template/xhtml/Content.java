...
/**
 * Model object adapter. Templates engine operates upon application model used as source for dynamic content. But model object
 * is, and must be, business oriented and is quite possible to exist documents or views with needs not covered directly by
 * model. This class is used exactly for that: it is an adapter for model used to synthesize properties not directly supplied by
 * it. So, strictly speaking, templates engine does not directly acts upon model object but on this content adapter.
 * <p>
 * This class supplies getters for miscellaneous types: string, object, list, array, map but all getters uses
 * {@link #getValue(Object, String)} which on its turn retrieves fields from model. Now is the interesting part: if model field
 * is missing value getter uses this class instance with a getter derived from searched property name. And since this class does
 * not offer per se any special getter it relies on subclasses.
 * 
 * <pre>
 *  class Person {
 *      int id;
 ...
 * </pre>
 * 
 * In above snippet we have a link with <em>href</em> operator having <em>link</em> as operand, in this case a property path.
 * Operator requests from content instance a property with name <em>link</em>. Content adapter searches person instance, the
 * model in this case, and on no such field create a getter with the name <em>getLink</em> and use it against content instance,
 * this time with success.
 * <p>
 * Content adapter does use <em>property path</em> abstraction to denote a specific content value. This name hides two concepts:
 * property and path.
 * <ul>
 * <li>Property is used to designates a value inside an object. Is is more generic than object field since it covers content
 * synthetic getters too, as shown above. Also a property name can be a numeric index, if object instance is an array or list.
 * This way both object and array like instances are accessible with the same property abstraction:
 * 
 * <pre>
 ...
 * </pre>
 * 
 * <li>In this class acception an object is a graph of values and property path is simply a list of path components separated by
 * dots. For example, in below snippet the property path <em>car.wheels.1.manufacturer</em> designates the manufacturer of the
 * second wheel from car while <em>car.model</em> designates, you guess, car model.
 * 
 * <pre>
 *  class Car {
 *      String model;
 ...
 * </pre>
 * 
 * Property path can be absolute, when begin with dot, or relative. An absolute path refers to content adapter root, that is,
 * the wrapped model. A relative one uses a scope object; this scope object is always present where property path is present too
 * and there are operators that change this scope object.
 * </ul>
 * 
...
public class Content {
	...
	/** Model object. This value holds application specific dynamic content and is the root of content adapter property paths. */
	protected Object model;

	/**
	 * Construct content instance.
	 * 
	 * @param converterRegistry converter manager,
	...
	/**
	 * Retrieve content object. Delegates {@link #getValue(Object, String)} to obtain the requested value. If value is null warn
	 * the event; in any case return value.
	 * 
	...
	/**
	 * Test if value is empty. Delegates {@link #getValue(Object, String)} and returns true if found value fulfill one of the
	 * next conditions:
	 * <ul>
	 * <li>null
	 * <li>boolean false
	...
	/**
	 * Get value converted to string. Delegates {@link #getValue(Object, String)} to obtain the requested value; if is null log
	 * warning and returns null value. Now, value can be of any type and need to be converted to string, as follow:
	 * <ul>
	 * <li>if format is not null returns its invocation result,
	 * <li>at this point value should be a {@link Types#isPrimitiveLike(Object) sudo-primitive} or have converter, otherwise
	 * fatal fail
	 ...
	 * <li>try to use {@link #converterManager}, throwing templates exception if fail.
	 * </ul>
	 * 
	 * @param scope scope object,
	 * @param propertyPath object property path,
	 ...
	 * </ul>
	 * Value can be about anything: primitives or aggregates. Anyway, there is distinction between not found value and a null
	 * one. First condition is known as undefined value and throws content exception; null value denotes an existing one but not
	 * initialized. Finally, this method uses {@link #getObjectProperty(Object, String)} to actually process path components in
	 * sequence.
	 * 
	...
	/**
	 * Get object property. This helper method is the work horse of all content getters. An object property is not limited to
	 * object field; it also includes array and list items, content instance getters and super-classes, as follow:
	 * <ul>
	...
	/**
	 * Return object property value using content getter. By convention accessor name is <code>get</code> concatenated with
	 * title case property name. If content has not such method throws content exception.
	 * 
